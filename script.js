...
function selectLocation(loc) {
    const adultData = (loc === 'Semenanjung') ? dataAdultSemenanjung : dataAdultBorneo;
    const kidsData = (loc === 'Semenanjung') ? dataKidsSemenanjung : dataKidsBorneo;
    ...
    // Kemaskini Tajuk
    document.getElementById('locTitleAdult').innerText = `DEWASA (${loc.toUpperCase()})`;
    document.getElementById('locTitleKids').innerText = `KIDS (${loc.toUpperCase()})`;

    // Animasi Peralihan
    document.getElementById('locationSection').classList.add('exit-up');
    setTimeout(() => {
        document.getElementById('locationSection').style.display = 'none';
        const ps = document.getElementById('priceListSection');
        ps.style.display = 'block';
        setTimeout(() => ps.classList.add('show'), 50);
    }, 400);
}
